...
import { tryUseNuxt, useLogger, resolveFiles, useNuxt, resolvePath as resolvePath$1, defineNuxtModule, findPath, addPlugin, addTemplate, addTypeTemplate, addComponent, updateTemplates, useNitro, addBuildPlugin, addImportsSources, tryResolveModule, isIgnored, resolveAlias, addPluginTemplate, addVitePlugin, normalizeModuleTranspilePath, resolveNuxtModule, resolveIgnorePatterns, logger as logger$1, createResolver, importModule, nuxtCtx, addWebpackPlugin, addServerPlugin, installModule, addRouteMiddleware, loadNuxtConfig, normalizeTemplate, compileTemplate, normalizePlugin, templateUtils } from '@nuxt/kit';
...
import { parse as parse$1, walk as walk$2, ELEMENT_NODE } from 'ultrahtml';
...
function parseAndWalk(code, _sourceFilename, callback) {
  ...
  declarePattern(pattern, parent) {
    ...
    for (const identifier of identifiers) {
      this.declareIdentifier(
        identifier.name,
        parent.type === "VariableDeclaration" ? new VariableNode(identifier, parent) : parent.type === "CatchClause" ? new CatchParamNode(identifier, parent) : new FunctionParamNode(identifier, parent)
      );
    }
  }
...
async function resolvePagesRoutes(nuxt = useNuxt()) {
...
const metaCache$1 = {};
async function getRouteMeta(contents, absolutePath, extraExtractionKeys = []) {
...
const ruleCache = {};
async function extractRouteRules(code, path) {
          ...
          walk(meta, {
            enter(node2, parent) {
              ...
              if (isStaticIdentifier(node2.name)) {
                ...
                processDeclaration(scopeTracker.getDeclaration(node2.name));
              }
            }
          });
...
const schemaTemplate = {
  ...
  getContents: async ({ nuxt }) => {
    ...
    const modules = nuxt.options._installedModules.filter((m) => m.meta && m.meta.configKey && m.meta.name && !m.meta.name.startsWith("nuxt:") && m.meta.name !== "nuxt-config-schema").map((m) => [genString(m.meta.configKey), getImportName(m.entryPath || m.meta.name), m]);
