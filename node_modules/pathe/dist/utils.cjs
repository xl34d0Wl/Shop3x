...
const path = require('./shared/pathe.DztlcRRt.cjs');
...
function resolveAlias(path$1, aliases) {
  ...
  for (const [alias, to] of Object.entries(aliases)) {
    ...
    if (hasTrailingSlash(_path[_alias.length])) {
      return path.join(to, _path.slice(alias.length));
    }
  }
  return _path;
}
function reverseResolveAlias(path$1, aliases) {
  const _path = path.normalizeWindowsPath(path$1);
  aliases = normalizeAliases(aliases);
  const matches = [];
  for (const [to, alias] of Object.entries(aliases)) {
    ...
    const _alias = hasTrailingSlash(alias) ? alias.slice(0, -1) : alias;
    if (hasTrailingSlash(_path[_alias.length])) {
      matches.push(path.join(to, _path.slice(alias.length)));
    }
  }
