...
import { p as path } from '../shared/nuxi.BZI8rqcO.mjs';
...
async function installMacos(to, version = CLOUDFLARED_VERSION) {
  ...
  fs__default.renameSync(`${path$1.dirname(to)}/cloudflared`, to);
  return to;
}
...
function download(url, to, redirect = 0) {
  ...
  return new Promise((resolve, reject) => {
    if (!fs__default.existsSync(path$1.dirname(to))) {
      fs__default.mkdirSync(path$1.dirname(to), { recursive: true });
    }
    let done = true;
...
function startCloudflaredTunnel(options = {}) {
  ...
  let urlResolver = () => undefined;
  let urlRejector = () => undefined;
