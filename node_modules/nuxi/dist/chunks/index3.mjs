...
const nativeAgents = { http: vt, https: https };
...
const stream = defineProxyMiddleware(
  (req, socket, options, server, head, callback) => {
    ...
    const proxyReq = (isSSL.test(options.target.protocol) ? https : vt).request(setupOutgoing(options.ssl || {}, options, req));
    if (server) {
...
class ProxyServer extends EventEmitter {
  ...
  listen(port, hostname) {
    ...
    this._server = this.options.ssl ? https.createServer(this.options.ssl, closure) : vt.createServer(closure);
    if (this.options.ws) {
