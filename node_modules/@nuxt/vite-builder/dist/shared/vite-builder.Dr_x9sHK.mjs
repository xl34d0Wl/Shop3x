...
import { dirname, resolve, normalize, isAbsolute, relative, join } from 'pathe';
import { useNuxt, useNitro, logger, resolvePath, tryImportModule, isIgnored, addVitePlugin } from '@nuxt/kit';
...
import { toNodeListener, createApp, defineEventHandler, defineLazyEventHandler, eventHandler, createError, appendCorsPreflightHeaders, appendCorsHeaders } from 'h3';
...
async function buildServer(ctx) {
  ...
  if (!ctx.nuxt.options.dev) {
    const runtimeDependencies = await tryImportModule("nitropack/package.json", {
    ...
    if (Array.isArray(serverConfig.ssr.external)) {
      serverConfig.ssr.external.push(
        ...
        "unstorage",
        "hookable",
        ...ctx.nuxt._nitro.options.inlineDynamicImports ? ["vue", "@vue/server-renderer", "@unhead/vue"] : [],
        ...runtimeDependencies
      );
    }
  }
        ...
        build: {
          ...
          rollupOptions: {
            output: {
              ...
              assetFileNames: nuxt.options.dev ? void 0 : (chunk) => withoutLeadingSlash(join(nuxt.options.app.buildAssetsDir, `${sanitizeFilePath(filename$1(chunk.name))}.[hash].[ext]`))
            }
          },
          watch: {
